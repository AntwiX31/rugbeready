<template>
  <div class="container">
    <div class="row mt-50 mb-50">
      <div class="col">
        <h2 class="text-center">Zebi</h2>
        <line-chart
          v-if="arrRandom.length > 0"
          :chartData="arrRandom"
          :options="chartOptions"
          :chartColors="positiveChartColors"
          label="Gayness of Zebi"
        />
        <line-chart
          v-if="arrRandom.length > 0"
          :chartData="arrRandom2"
          :options="chartOptions"
...
export default {
  ...
  async created() {
    const { data } = await axios.get(
      "https://api.covidtracking.com/api/us/daily"
    );
    for (var i = 100; i > 10; i--) {
      this.arrRandom.push({ date: i, total: i });
      this.arrRandom2.push({ date: i, total: i-(1.5*i) });
    }
    data.forEach((d) => {
